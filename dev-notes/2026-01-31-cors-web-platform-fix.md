...
date: 2026-01-31
category: Backend
title: 웹 플랫폼을 위한 CORS 설정 수정 (3chan.kr 도메인 지원)
...
## 개요
웹 앱(`http://3chan.kr/app/`)에서 API 요청 시 CORS 에러가 발생하는 문제를 해결했습니다. Progress Service와 Media Service의 CORS 설정에 `3chan.kr` 도메인을 추가하여 개발 환경에서도 도메인을 통한 접근을 허용했습니다.
...
- `http://3chan.kr:3007` (도메인 기반 개발 서버) ❌ **CORS 에러**
- `http://3chan.kr/app/` (도메인 기반 Nginx) ❌ **CORS 에러**

웹 앱이 `3chan.kr` 도메인에서 실행될 때 Progress/Media API를 호출하면 CORS 에러가 발생했습니다.
...
```go
    ...
    config.AllowedOrigins = []string{
        ...
        "http://127.0.0.1",
        "http://127.0.0.1:3007",
        "http://3chan.kr:3007", // Dev server port
        "http://3chan.kr",      // Dev domain (nginx)
    }
}
```
...
**환경 감지**:
- `NODE_ENV=production`: 프로덕션 도메인만 허용 (`https://3chan.kr`)
...
### Go 설정 파일 (수정)
- `/services/progress/config/cors.go` - `3chan.kr` 도메인 추가
- `/services/media/config/cors.go` - `3chan.kr` 도메인 추가
...
```bash
# 3chan.kr:3007 origin 테스트
curl -X OPTIONS http://localhost:3003/api/progress/user/1 \
  -H "Origin: http://3chan.kr:3007" \
  -H "Access-Control-Request-Method: GET" \
  -v

# 응답:
# HTTP/1.1 204 No Content
# Access-Control-Allow-Origin: http://3chan.kr:3007
...
curl -X OPTIONS http://localhost:3004/media/images/test.jpg \
  -H "Origin: http://3chan.kr" \
  -H "Access-Control-Request-Method: GET" \
  -v

# 응답:
# HTTP/1.1 204 No Content
# Access-Control-Allow-Origin: http://3chan.kr
# Access-Control-Allow-Credentials: true
```
...
### 웹 앱 접속 방법
1. `http://3chan.kr:3007` 또는 `http://localhost:3007`
...
- 문제: `localhost`만 지원, `3chan.kr` 도메인 미지원

### 이번 업데이트
- 2026-01-31: `3chan.kr` 도메인을 개발 환경 CORS에 추가
...
**요약**: 웹 플랫폼이 `3chan.kr` 도메인에서 정상 동작하도록 CORS 설정을 확장했습니다. 모든 테스트 통과 및 배포 완료.
