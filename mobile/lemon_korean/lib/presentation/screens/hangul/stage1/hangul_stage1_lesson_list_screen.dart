...
import 'stage1_lesson_content.dart';

/// Shows the lessons of Stage 1 (기본 모음) with completion status and lemon rewards.
class HangulStage1LessonListScreen extends StatelessWidget {
  const HangulStage1LessonListScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('1단계: 기본 모음')),
      body: Consumer<HangulProvider>(
        builder: (context, hangul, _) {
          return ListView.separated(
            padding: const EdgeInsets.all(16),
            itemCount: stage1Lessons.length,
            separatorBuilder: (_, __) => const SizedBox(height: 12),
            itemBuilder: (context, index) {
              final lesson = stage1Lessons[index];
  ...
  bool _isPreviousCompleted(HangulProvider hangul, int index) {
    ...
    final prevLesson = stage1Lessons[index - 1];
