...
import 'stage7_lesson_content.dart';

/// Shows the lessons of Stage 7 (된소리/거센소리) with completion status.
class HangulStage7LessonListScreen extends StatelessWidget {
  const HangulStage7LessonListScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('7단계: 된소리/거센소리')),
      body: Consumer<HangulProvider>(
        builder: (context, hangul, _) {
          return ListView.separated(
            padding: const EdgeInsets.all(16),
            itemCount: stage7Lessons.length,
            separatorBuilder: (_, __) => const SizedBox(height: 12),
            itemBuilder: (context, index) {
              final lesson = stage7Lessons[index];
  ...
  bool _isPreviousCompleted(HangulProvider hangul, int index) {
    ...
    final prevLesson = stage7Lessons[index - 1];
