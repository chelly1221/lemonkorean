...
import 'stage3_lesson_content.dart';

/// Shows the lessons of Stage 3 (ㅐ/ㅔ 모음) with completion status and lemon rewards.
class HangulStage3LessonListScreen extends StatelessWidget {
  const HangulStage3LessonListScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('3단계: ㅐ/ㅔ 모음')),
      body: Consumer<HangulProvider>(
        builder: (context, hangul, _) {
          return ListView.separated(
            padding: const EdgeInsets.all(16),
            itemCount: stage3Lessons.length,
            separatorBuilder: (_, __) => const SizedBox(height: 12),
            itemBuilder: (context, index) {
              final lesson = stage3Lessons[index];
  ...
  bool _isPreviousCompleted(HangulProvider hangul, int index) {
    ...
    final prevLesson = stage3Lessons[index - 1];
