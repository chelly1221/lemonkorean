...
import 'stage9_lesson_content.dart';

/// Shows the lessons of Stage 9 (받침 확장) with completion status.
class HangulStage9LessonListScreen extends StatelessWidget {
  const HangulStage9LessonListScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('9단계: 받침 확장')),
      body: Consumer<HangulProvider>(
        builder: (context, hangul, _) {
          return ListView.separated(
            padding: const EdgeInsets.all(16),
            itemCount: stage9Lessons.length,
            separatorBuilder: (_, __) => const SizedBox(height: 12),
            itemBuilder: (context, index) {
              final lesson = stage9Lessons[index];
  ...
  bool _isPreviousCompleted(HangulProvider hangul, int index) {
    ...
    final prevLesson = stage9Lessons[index - 1];
