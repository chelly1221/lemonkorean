...
import 'stage10_lesson_content.dart';

/// Shows the lessons of Stage 10 (복합 받침) with completion status.
class HangulStage10LessonListScreen extends StatelessWidget {
  const HangulStage10LessonListScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('10단계: 복합 받침')),
      body: Consumer<HangulProvider>(
        builder: (context, hangul, _) {
          return ListView.separated(
            padding: const EdgeInsets.all(16),
            itemCount: stage10Lessons.length,
            separatorBuilder: (_, __) => const SizedBox(height: 12),
            itemBuilder: (context, index) {
              final lesson = stage10Lessons[index];
  ...
  bool _isPreviousCompleted(HangulProvider hangul, int index) {
    ...
    final prevLesson = stage10Lessons[index - 1];
