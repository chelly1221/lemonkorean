...
import 'stage8_lesson_content.dart';

/// Shows the lessons of Stage 8 (받침 1차) with completion status.
class HangulStage8LessonListScreen extends StatelessWidget {
  const HangulStage8LessonListScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('8단계: 받침(종성) 1차')),
      body: Consumer<HangulProvider>(
        builder: (context, hangul, _) {
          return ListView.separated(
            padding: const EdgeInsets.all(16),
            itemCount: stage8Lessons.length,
            separatorBuilder: (_, __) => const SizedBox(height: 12),
            itemBuilder: (context, index) {
              final lesson = stage8Lessons[index];
  ...
  bool _isPreviousCompleted(HangulProvider hangul, int index) {
    ...
    final prevLesson = stage8Lessons[index - 1];
