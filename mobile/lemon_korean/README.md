...
다국어 한국어 학습 앱 - Flutter 모바일 + 웹 애플리케이션 (6개 언어 지원)
...
│   │   ├── gop_service.dart           # GOP 서비스 진입점 (플랫폼 분기)
│   │   ├── gop_service_native.dart    # 임베딩 추출 (ONNX, 모바일)
│   │   ├── gop_service_stub.dart      # 웹 스텁 (null 반환)
...
### Hangul Learning Module (2026-02-03, 2026-02-28 확장)

12단계(Stage 0~11) 체계적 한글 커리큘럼. 80개 인터랙티브 레슨, 필기 애니메이션, 음성 비교. 전 단계 6개 언어 완전 로컬라이제이션 완료.
...
**i18n 구조** (2026-02-28):
- Stage 0~11 콘텐츠가 `const` 리스트에서 `getStageNLessons(AppLocalizations l10n)` 함수로 변환됨
- ARB 키 네이밍: `hangulS{N}L{M}Title`, `hangulS{N}L{M}Step{I}Title` 등
- ~1,227개 새 ARB 키 추가 (한글 단계 전용)

**Pronunciation Scoring** (2026-03-01, 임베딩 기반으로 전환):
- wav2vec2 임베딩 기반 코사인 유사도 채점 (GOP 음소별 채점에서 전환)
- 트리밍된 ONNX 모델: CTC 헤드 제거, 1024차원 은닉 상태 추출
- TTS 참조 임베딩: edge-tts 생성 + 평균 풀링 (152개 문자/단어)
- 점수 공식: 코사인 유사도 (0.30~0.85) → 0~100점 매핑
- 6단계 오디오 전처리: DC 제거 → 80Hz 고역통과 → 노이즈 게이트 → 프리엠퍼시스
- WAV 파싱: RIFF/WAVE 청크 유효성 검증, data 청크 탐색
- 최소 녹음 길이: 0.5초 (16kHz 기준 8000 샘플)
- 패키지: `onnxruntime` → `onnxruntime_v2` (v1.23.2, ConvInteger 지원)
...
### Voice Rooms (2026-02-10, 2026-03-01 안정화 완료)

LiveKit 기반 음성 대화방. 6가지 방 유형, Flame 게임 엔진 시각화, 대화 주제 자동 제공. 3차 버그픽스로 안정화 완료.
...
- 방 목록 참가자에 `skin_color` + `equipped_items` 포함 (캐릭터 표시)
- broadcast StreamController 이벤트 손실 수정 (생성자 초기 스피커 전달)
- 원격 캐릭터 화면 리사이즈 시 스케일링 수정
- 6개 소켓 핸들러에 room_id 유효성 검증 추가

**Backend Integration**:
- **10 API endpoints** in SNS Service (`/api/sns/voice-rooms/*`)
- **LiveKit token** generated server-side on join (대상 사용자에게만 전송)
...
### VoiceRoomProvider (2026-02-10, 2026-03-01 안정화)
- 음성 대화방 목록/상세
- LiveKit 연결 및 토큰 관리
- 방 생성/입장/퇴장 (동시 작업 가드)
- 뮤트 토글
...
- **총 키 수**: 2,800+개
- **카테고리**: UI, 레슨 7단계, 한글 Stage 0~11, 음성 대화방, 커뮤니티, DM, 설정, 오류 메시지, 알림 등
