...
**Pronunciation Scoring** (2026-02-27):
- GOP-only 파이프라인 (Whisper 완전 제거)
- 점수 공식: `GOP_avg * 0.40 + syllable_position_weighted * 0.60`
- 위치 가중치: 초성 40%, 중성 40%, 종성 20%
- Sigmoid 정규화 (선형 매핑 대체)
- 복잡 받침 처리: `complexFinalToSimple` 맵 + `simplifyForGop()` 함수
- 최소 녹음 길이 검증: 0.3초 (16kHz 기준 4800 샘플)
